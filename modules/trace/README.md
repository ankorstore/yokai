...
Since The `TracerProviderFactory` use the [resource.Default()](https://pkg.go.dev/go.opentelemetry.io/otel/sdk/resource#Default) by default, you can use:
...
If no tracer provider is found in context, the [global tracer](https://github.com/open-telemetry/opentelemetry-go/blob/main/trace.go) will be used.
...
- `OtlpGrpc`: to async send traces to [OTLP/gRPC](https://opentelemetry.io/docs/specs/otlp/#otlpgrpc) collectors (ex: [Jaeger](https://www.jaegertracing.io/), [Grafana](https://grafana.com/docs/tempo/latest/configuration/grafana-agent/#grafana-agent), etc.)
